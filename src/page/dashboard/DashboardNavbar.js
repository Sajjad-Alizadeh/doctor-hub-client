...
function DashboardNavbar({
    ...
    const analysisIconStyle = isAnalysisSelected ? ' h-7 w-7 ' : ' h-6 w-6 ';
    ...
    const analysisStyle = isAnalysisSelected ? ' my-3 h-[50px] w-[90%] text-white text-m font-bold shadow-emerald-500 cursor-pointer bg-emerald-400 rounded-lg shadow-lg shadow-emerald-700 flex flex-row '
        : 'h-[50px] w-[90%] my-3 text-gray-200 text-m  cursor-pointer rounded-lg   flex flex-row ';
    ...
    return (
        <>
            <div
                ...
                <div
                    className={analysisStyle + ' items-start justify-start rtl p-2 '} onClick={selectAnalysis}>
                    <img src={userWhiteIcon} className={'mx-3' + analysisIconStyle}/>
                    تحلیل
                </div>
                <div
